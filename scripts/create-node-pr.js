...
const { CWD, run, spawn, git, fs, gh, npm } = require('./util.js')

// this script expects node to already be cloned to a directory
// at the cli root named "node"
const NODE_REMOTE = 'origin'
const NODE_DIR = join(CWD, 'node')
const NPM_DEP = join('deps', 'npm')
const NPM_DIR = join(NODE_DIR, NPM_DEP)
...
const createNodeTarball = async ({ mani, registryOnly, tag, dir: extractDir }) => {
  ...
  // remove *.pyc files from node-gyp
  const gyp = await npm.query('#node-gyp').then(r => r[0].location)
  const gypFilter = (p) => minimatch(p, join(extractDir, gyp, '**', '*.pyc'))

  const tarOpts = pacote.DirFetcher.tarCreateOptions(mani)
  await tar.c({
    ...tarOpts,
    cwd: extractDir,
    file: tarball,
    filter: (path, stat) => {
      if (!tarOpts.filter(path, stat) || gypFilter(path)) {
        return false
      }
      return true
    },
...
const main = async (spec, opts) => withTempDir(CWD, async (tmpDir) => {
  ...
  const base = {
    ...
    remote: NODE_REMOTE,
    host: hgi.fromUrl(await gitNode('remote', 'get-url', NODE_REMOTE, { out: true })),
  }
  log.silly(base)
  ...
  await fs.clean(NPM_DIR)
  await tar.x({ strip: 1, file: tarball, cwd: NPM_DIR })

  await gitNode('add', '-A', NPM_DEP)
